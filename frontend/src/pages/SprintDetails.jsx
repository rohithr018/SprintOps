...
import { FiEdit, FiTrash2, FiChevronRight, FiFolder } from "react-icons/fi";
...
export default function SprintDetails() {
	...
	return (
		<div className="min-h-screen bg-gradient-to-br from-white via-white to-slate-100 px-4 md:px-6 lg:px-10 py-2 space-y-10">
			{/* HEADER (Glass Card) */}
			<header
				className="
			...
			>
				...
				<div className="flex items-center gap-3">
					<button
						...
						className="px-4 py-2 rounded-xl border border-slate-300 bg-white/80 shadow-sm hover:bg-white text-sm flex items-center gap-2"
					>
						<FiEdit /> Edit
					</button>
					...
					<button
						...
						className="px-4 py-2 rounded-xl bg-red-600 text-white shadow-sm hover:bg-red-700 text-sm flex items-center gap-2"
					>
						<FiTrash2 /> Delete
					</button>
				</div>
								...
								className={`px-4 py-1.5 rounded-full text-sm font-medium transition
									${
										active === t.id
											...
											: "bg-white/70 border border-slate-200 hover:bg-white text-slate-700"
									}`}
							>
								{t.label}
								{active === t.id && (
									<FiChevronRight className="inline-block ml-1 opacity-70" />
								)}
							</button>
						))}
					</div>
			...
			{/* CONTENT BLOCK (NO BLUR — FIXES DRAG DROP) */}
			<div className="rounded-xl p-1">
				{active === "tasks" && (
					<div className="mt-3">
						<TaskBoard sprintId={id} />
					</div>
				)}
				...
				{active === "analytics" && (
					<div className="p-6 rounded-xl bg-white/70 border border-slate-200 text-sm text-slate-600 shadow-inner">
